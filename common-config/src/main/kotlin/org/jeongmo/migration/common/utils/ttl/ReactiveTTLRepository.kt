...
/**
 * 비동기 방식의 TTL repository
 */
interface ReactiveTTLRepository {
     ...
     */
    fun save(key: String, value: Any, ttl: Long): Mono<Boolean>

    /**
     * 없는 경우에만 TTL을 이용하여 값을 저장
     *
     * @param key 확인할 key
     * @param value 없는 경우 저장할 값
     * @param ttl 해당 데이터를 유지할 시간 (sec)
     * @return 있는 경우 이미 있는 값, 없는 경우 저장 후 null 반환
     */
    fun saveIfAbsent(key: String, value: Any, ttl: Long): Mono<Any?>
     ...
     */
    fun has(key: String): Mono<Boolean>
     ...
     */
    fun <T> findByKey(key: String, clz: Class<T>): Mono<T?>
     ...
     */
    fun deleteValue(key: String): Mono<Boolean>
}