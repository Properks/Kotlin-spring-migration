...
class MemberService(
    ...
    override fun createMember(request: CreateMemberRequest): CreateMemberResponse {
        ...
        return CreateMemberResponse.fromDomain(member).also { logger.info("[SUCCESS_CREATE_MEMBER] member | id: ${it.id}") }
    }
    ...
    override fun findById(id: Long): MemberInfoResponse {
        ...
        ).also { logger.info("[FIND_DOMAIN] member | id: ${it.id}") }
    }
    ...
    override fun verifyMember(request: VerifyMemberRequest): VerifyMemberResponse? {
        ...
        return foundMember?.let {
            if (passwordEncoder.matches(request.password, it.password)) {
                return VerifyMemberResponse.fromDomain(member = it).also { logger.info("[SUCCESS_VERIFY] member | id: ${it.id}") }
            }
            else {
    ...
    override fun updateMemberInfos(id: Long, request: UpdateMemberInfoRequest): UpdateMemberInfoResponse {
        ...
        return UpdateMemberInfoResponse.fromDomain(updatedMember).also { logger.info("[SUCCESS_UPDATE] member | id: ${it.id}") }
    }
    ...
    override fun deleteMember(id: Long) {
        try {
            ...
            logger.warn("[FAIL_DELETE] member | ServerApplicationException: ${e.message}", e)
            throw e
        } catch (e: Exception) {
            logger.error("[FAIL_DELETE] member | Unknown Error: ${e.message}", e)
